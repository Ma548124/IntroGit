...
//  Dungeon-Game
//
//  Created by Angel Munoz on 4/19/26.
//
...
//in the game it would be used in a teleporter or using to put a code of the struct
...
struct GameObject {
    ...
    GameObject() : type(ObjectType::EMPTY), hp(0), attack(0),defense(0),value(0),switchIndex(0),floortype(),active(true){}
};
...
//Here is the class Inventory that would help the player use in the game
class Inventory {
private:
    int keys = 0;
    int dimonds = 0;
...
bool processMove(vector<vector<char>>& dungeon, Player& player, int dx, int dy, const vector<pair<pair<int, int>, pair<int, int>>>& teleporters) {
    ...
    char target = dungeon[newY][newX]; // char target = dungeon[newX][newY];
    if (target == '#') {
    ...
    dungeon[player.y][player.x] = ' '; // clear old position
    player.x = newX;
    player.y = newY;
...
public:
    ...
    int run() {
        ...
        game_setTileType({ 3,2,0,0 });
        game_setTileType({ 1,5,0,0 });
        game_setTileType({ 2,4,0,0 });
        game_setTileType({ 4,6,0,0 });
        /*game_setTileType({3,2});
        game_setTileType({ 1,5 });
        game_setTileType({ 2,4 });
        game_setTileType({ 4,6 });*/

        // Main menu loop that would used to solved in big loop in the section
        bool running = true;
        while (running) {
            ...
            if (choice == 1) {
                ...
                // Game loop again the code goes when it running in a loop based
                ...
                // Design a dungeon by your ownself in the coding
